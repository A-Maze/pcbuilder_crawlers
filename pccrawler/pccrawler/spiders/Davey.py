...
class Davey(CrawlSpider):
    ...
    def parse_item(self, response):
        for sel in response.xpath(
            ...
            item['omschrijving'] = sel.xpath(
                './/[contains(concat(" ", normalize-\
                    space(@class), " "), "kenmerken \
            ")]/li/text()').extract()[0]
            item['prijs'] = sel.xpath(
                './/span[contains(concat(" ", normalize\
                    -space(@class), " "), "prijs_zicht "\
            )]/text()').extract()[0]
            item['image'] = sel.xpath('ul/li/a/img/@src').extract()[0]
            item['categorie'] = sel.xpath(
                '//div[@id="zoek-titel"]/h1/text()').extract()[0]
            yield item
